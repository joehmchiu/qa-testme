...
locals {
  ...
  vm = format("%s-%03d", var.prefix, var.instance_count)
}
...
data "azurerm_subnet" "win" {
  name                 = "${var.vnet}-subnet-${local.rgfix}"
  resource_group_name  = data.azurerm_resource_group.win.name
  virtual_network_name = "${var.vnet}-vnet-${local.rgfix}"
}

resource "azurerm_public_ip" "win" {
  name                = "${var.prefix}-pip-${local.rgfix}"
  resource_group_name = data.azurerm_resource_group.win.name
...
resource "azurerm_network_security_group" "win" {
  name                = "${var.prefix}-nsg-${local.postfix}"
  resource_group_name = data.azurerm_resource_group.win.name
  location            = var.location
...
resource "azurerm_network_interface" "win" {
  name                = "${var.prefix}-nic-${local.postfix}"
  location            = var.location
  resource_group_name = data.azurerm_resource_group.win.name
...
resource "azurerm_windows_virtual_machine" "win" {
  ...
  admin_password      = "ComplxP${local.env}@ssw0rd!"
